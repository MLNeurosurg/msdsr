...
Credit is given to the following projects:
...
from msdsr.ldm.util import exists, default, mean_flat, count_params
from msdsr.ldm.modules.ema import LitEma
from msdsr.ldm.modules.distributions.distributions import normal_kl, DiagonalGaussianDistribution
from msdsr.ldm.models.autoencoder import VQModelInterface, IdentityFirstStage, AutoencoderKL
from msdsr.ldm.modules.diffusionmodules.util import make_beta_schedule, extract_into_tensor, noise_like
from msdsr.ldm.models.diffusion.ddim import DDIMSampler

from msdsr.ldm.factory import (instantiate_diffusion_model,
                               instantiate_first_stage_models,
                               instantiate_cond_model,
...
class DDPM(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Occasionally log images to tensorboard
        if batch_idx % 200 == 0:
            self.log_images_tb(
                self.log_images(batch,
                                plot_progressive_rows=False,
    ...
    def log_images_tb(self, log_im, tv):
        """Log images to tensorboard while training."""
        tb_logger = self.trainer.loggers[0]

        for k in log_im.keys():
            im = log_im[k].squeeze()
            if len(im.shape) == 4:
                im = im[0, ...]
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.msdsr_masking_range = msdsr_masking_range
    ...
    @torch.no_grad()
    def pred_2d(self, c, shape):
        """Predict 2D image for evaluation.

        Args:
            c: condition
            shape: shape of image

        Returns prediction over a 2D image with conditioning.
        """
        ddim_steps = 200
        use_ddim = True
        ddim_eta = 1
    ...
    def predict_step(self, batch, _):
        """Super-resolve images from low-res to high-res.

        Args:
            batch: batch of images to super-resolve

        Returns:
            dict of ground truth, predictions, paths to images, and z position
        """
        ...
        return [{
            ...
            "path": pa,
            "z_pos": zp.item()
        } for gt, pr, pa, zp in zip(x, x_samples, paths, z_pos)]
    ...
    def instantiate_first_stage(self, config):
        model = instantiate_first_stage_models(config)
    ...
    def instantiate_cond_stage(self, config):
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                ...
                model = instantiate_cond_model(config)
    ...
    def get_input(self,
        ...
        # Mask out a random subset of rows to use as conditioning
        if self.model.conditioning_key == "msdsr":
            cutoff = random.randint(*self.msdsr_masking_range)
            rp = torch.stack(
                [torch.randperm(x.shape[2]) for _ in range(x.shape[0])])
            x_mask = rp[:, :cutoff].sort().values
            c_mask = rp[:, cutoff:].sort().values
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                ...
                # Collect the conditioning slices based on mask
                if self.model.conditioning_key == "msdsr":
                    xc = torch.stack([xi[:, mi] for xi, mi in zip(x, c_mask)])
                else:
                    xc = x

            # Stack conditioned rows together to form final 2D condition
            if self.model.conditioning_key == "msdsr":
                c = torch.stack([
                    self.get_learned_conditioning(xc_.to(self.device))
                    for xc_ in xc
                ])
            elif not self.cond_stage_trainable or force_c_encode:
        ...
        # Export embedding, [condition, x mask, condition mask]
        if self.model.conditioning_key == "msdsr":
            out = [z, [c, x_mask, c_mask]]
        else:
            out = [z, c]
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if isinstance(cond, dict):
            ...
            if self.model.conditioning_key in {"crossattn"}:
                key = 'c_crossattn'
            elif self.model.conditioning_key in {'concat', "msdsr"}:
                # Use c_concat key for msdsr
                key = 'c_concat'
            else:
                raise ValueError("unacceptable conditioning_key")
    ...
    def _predict_eps_from_xstart(self, x_t, t, pred_xstart):
        return (
            extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) *
            x_t - pred_xstart) / extract_into_tensor(
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # Interlace conditioned rows with ground truth rows for prediction
        if self.model.conditioning_key == "msdsr":
            if self.parameterization == "x0":
                for xi, ci, ii in zip(target, cond[0], cond[2]):
                    xi[:, ii, :] = ci
            elif self.parameterization == "eps":
                for xi, ci, ii in zip(target, cond[0], cond[2]):
                    xi[:, ii, :] = 0
        ...
        loss_simple = loss_simple.mean(
            dim=tuple(torch.arange(1, len(loss_simple.shape))))
        ...
        loss_vlb = loss_vlb.mean(
            dim=tuple(torch.arange(1, len(loss_vlb.shape))))
    ...
    def log_images(self,
        ...
        if sample:
            ...
            # Remove extra dimension for msdsr logging
            if self.model.conditioning_key == "msdsr":
                x_samples = self.decode_first_stage(samples.squeeze())
    ...
    def configure_optimizers(self):
        ...
        opt = torch.optim.AdamW(params, lr=lr)
        if self.use_scheduler:
            scheduler = instantiate_lr_scheduler(self.scheduler_config)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.diffusion_model = instantiate_diffusion_model(diff_model_config)
        self.conditioning_key = conditioning_key
        assert self.conditioning_key in [
            None, 'concat', 'crossattn', 'hybrid', 'adm', "msdsr"
        ]
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        ...
        elif self.conditioning_key == "msdsr":
            # Insert conditioned rows into image for guided prediction
            for xi, ci, ii in zip(x, c_concat[0], c_concat[2]):
                xi[:, ii, :] = ci
            out = self.diffusion_model(x, t)
